...
LRESULT WINAPI WinProc(HWND hWnd,UINT Msg,WPARAM wParam,LPARAM lParam)
{
	...
	case WM_PAINT:
		...
		//本例中是单线、水平居中和竖直居中。
		DrawText(hDC,"P1机场调度完成，请在out.txt中查看过程",-1,&rect,DT_SINGLELINE|DT_CENTER|DT_VCENTER);
		//EndPaint就是做些收尾的工作了。
		EndPaint(hWnd,&paint);
		break;
    ...
    bool acceptable=false;
     do{
         ...
         if (acceptable&&arrival_rate + departure_rate>1.0)
             cerr << "Safety Warning: This airport will become satu raed." <<endl;
     }while (!acceptable);
}
...
int WINAPI WinMain(HINSTANCE hInstance,HINSTANCE hPrevInstance,LPSTR lpCmdLine,int nShowCmd) {
   ...
   char *cName = "myWindow";
    ...
    hWnd = CreateWindowEx(WS_EX_CLIENTEDGE,cName,"P1单跑道版本",WS_OVERLAPPEDWINDOW,
